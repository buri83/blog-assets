...
(async () => {
    ...
    for (const state of ["S0", "S1", "S2", "S3"] as State[]) {
        console.log(`Q(${state},Right) = ${qTable.get(getKey(state, "Right"))?.toFixed(3)}`);
        console.log(`Q(${state},Left) = ${qTable.get(getKey(state, "Left"))?.toFixed(3)}`);
    }
})();
